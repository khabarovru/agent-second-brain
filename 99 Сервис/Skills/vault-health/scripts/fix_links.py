...
def suggest_fix(broken_from: str, broken_to: str, stem_index: dict) -> tuple[str | None, str]:
    ...
    for old_prefix, new_prefix in path_mappings.items():
        if target.startswith(old_prefix):
            ...
            try:
                if (VAULT_PATH / f"{new_target}.md").exists():
                    return new_target, "replace"
            except OSError:
                pass
    ...
    # Attachments/Files → remove wikilink (they're embeds, not graph nodes)
    # BUT: if the target has a file extension (e.g. .xlsx, .pdf), it's a legitimate
    # file embed and should NOT be removed — preserve the wikilink as-is
    if target.startswith("attachments/") or target.startswith("95 Файлы/"):
        if "." in Path(target).name:  # has extension → keep link
            return None, "none"
        return None, "remove"

    # 10 Заметки/ references that don't exist → check if stem exists
    if target.startswith("10 Заметки/"):
    ...
    if "/" in target:
        ...
        try:
            ...
            # Handle "File name too long" error (>255 chars)
            # If path is too long, file definitely doesn't exist → remove link
            return None, "remove"
...
def apply_remove(file_path: Path, old_link: str) -> bool:
    ...
    ВАЖНО: НЕ трогает embed-ссылки ![[...]] — они остаются как есть.
    """
    content = file_path.read_text(encoding="utf-8")
    ...
    # Но только если перед [[ нет ! (иначе это embed ![[]])
