...
# PATH for systemd (openclaw, uv, npx, node)
...
if [ "$DAILY_SIZE" -lt 50 ]; then
    echo "ORIENT: daily/$TODAY.md is empty ($DAILY_SIZE bytes) — skipping openclaw processing"
    echo "ORIENT: Running graph rebuild only"

    # Still rebuild graph and commit
    cd "$VAULT_DIR"
    uv run /home/khabarovru/.openclaw/skills/graph-builder/scripts/analyze.py || echo "Graph rebuild failed (non-critical)"
    cd "$PROJECT_DIR"
...
# Extract JSON from output (openclaw may add text around it)
...
echo "=== Memory decay ==="
uv run /home/khabarovru/.openclaw/skills/agent-memory/scripts/memory-engine.py decay . || echo "Memory decay failed (non-critical)"
cd "$PROJECT_DIR"
